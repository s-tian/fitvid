...
from fitvid.depth_utils import depth_to_rgb_im


def build_visualization(gt, pred, gt_depth, gt_depth_pred, pred_depth, rgb_loss, depth_loss_weight, depth_loss):
    tlen = gt.shape[1]

    # convert depth images to RGB visualizations
    cmap = plt.get_cmap('jet_r')
    gt_depth = np.moveaxis(depth_to_rgb_im(gt_depth.cpu().numpy(), cmap), 4, 2)
    gt_depth_pred = np.moveaxis(depth_to_rgb_im(gt_depth_pred.cpu().numpy(), cmap), 4, 2)
    pred_depth = np.moveaxis(depth_to_rgb_im(pred_depth.cpu().numpy(), cmap), 4, 2)
    ...
    image_rows = [gt.cpu().numpy() * 255, pred.cpu().numpy() * 255, pred_depth, gt_depth, gt_depth_pred,
...
def generate_text_square(text, size=(64, 64)):
    img = np.ones(shape=(512, 512, 3), dtype=np.int16)
    cv2.putText(img=img, text=text, org=(50, 250), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=2.5,
