...
class RandomResizedCropAntialias(torch.nn.Module):
    def __init__(self, output_size, height_minimum_ratio=0.8):
        ...
        self.max_wiggle = self.output_size[0] - math.ceil(self.height_minimum_ratio * self.output_size[0]) # maximum amount of wiggle in pixels

        self.resize = Resize(self.output_size, interpolation=InterpolationMode.BILINEAR, antialias=True)
    ...
    def crop(self, x):
        ...
        assert starts[0] + crop_size <= x.shape[-2] and starts[1] + crop_size <= x.shape[-1]
        return x[..., starts[0]:starts[0]+crop_size, starts[1]:starts[1]+crop_size]
...
class FitVidAugment(torch.nn.Module):
    ...
    def __init__(self, image_size=(64, 64), crop_height_min_ratio=0.8):
        ...
        self.rand_augment = RandAugment(num_ops=1,
                                        magnitude=5)
        # Scale to at most 0.8 of either dimension of the frame (0.64 of the total area).
        # Always maintain the same aspect ratio.
        self.resized_crop = RandomResizedCrop(image_size, scale=(crop_height_min_ratio**2, 1.0), ratio=(1.0, 1.0))
