...
class DepthPredictor(nn.Module):
    def __init__(
        self, depth_model_type, pretrained_weight_path, input_size, freeze_model=True
    ):
        super().__init__()
    ...
    def load_pretrained_depth_model(self):
        ...
        self.register_buffer(
            "depth_head_mean", torch.Tensor([0.485, 0.456, 0.406])[..., None, None]
        )
        self.register_buffer(
            "depth_head_std", torch.Tensor([0.229, 0.224, 0.225])[..., None, None]
        )
        return depth_model
    ...
    def forward(self, pred_frame, time_axis=True):
        ...
        pred_frame_norm = (
            pred_frame - self.depth_head_mean
        ) / self.depth_head_std  # normalize as done for pretrained MiDaS
        ...
        if self.input_size != pred_frame.shape[-1]:
            pred_frame_norm = torch.nn.Upsample(
                scale_factor=self.input_size // pred_frame_norm.shape[-1]
            )(pred_frame_norm)
        depth_pred = self.depth_head(pred_frame_norm)[:, None].float()
        # normalize to [0, 1]
        if self.input_size != pred_frame.shape[-1]:
            depth_pred = torch.nn.functional.interpolate(
                depth_pred, size=pred_frame.shape[-2:]
            )
        if time_axis:
